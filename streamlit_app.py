...
# ========== KALKULATOR ==========
    ...
    with tab3:
        st.markdown('<div class="rumus-box">Sulfat:\nKadar SO₄ (%) = (BM SO₄ / Mr BaSO₄) × [(W₂-W₀)/(W₁-W₀)] × 100 \nBM SO₄ = 96,06 ; Mr BaSO₄ = 233,39  →  Faktor = 0,4116</div>',unsafe_allow_html=True)
    ...
    with tab4:
        ...
        if st.button("Hitung Kadar Besi",type="primary"):
            kadar=(111.70/159.69)*((w2-w0)/(w1-w0))*100 if w1>w0 and w2>=w0 else -1
            st.success(f"✅ Kadar Fe = {kadar:.4f} %") if kadar>=0 else st.error("⚠️ Cek kembali data input!")
    with tab5:
        st.markdown('<div class="rumus-box">Barium:\nKadar Ba (%) = (Ar Ba / Mr BaCrO₄) × [(W₁-W₀)/Vol] × 100 \nAr Ba = 137,33 ; Mr BaCrO₄ = 253,32  →  Faktor = 0,5421</div>',unsafe_allow_html=True)
