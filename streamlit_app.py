...
========== KALKULATOR ==========
    ...
    with tab1:
        st.markdown('<div class="rumus-box">Kadar Air (%) = [( (W₁ - W₀) - (W₂ - W₀) ) / (W₁ - W₀)] × 100</div>',unsafe_allow_html=True)
    ...
    with tab5:
        st.markdown('<div class="rumus-box">Barium:\nKadar Ba (%) = (Ar Ba / BM BaCrO₄) × [(W₁-W₀)/Vol] × 100 \nAr Ba = 137,33 ; BM BaCrO₄ = 253,32</div>',unsafe_allow_html=True)
...
================================================
    ...
    with tab4:
        ...
        st.markdown("> **Penjelasan:** Fe₂O₃ berwarna **merah bata** (hematit). Inilah zat yang ditimbang akhir.")  

    # ---- REAKSI PERCOBAAN 5 ----  
    with tab5:
        st.subheader("Percobaan 5 – Reaksi dalam Pengendapan Seragam Barium")  

        st.markdown("**Reaksi 1: Hidrolisis Urea (kunci metode pengendapan seragam)**")  
        col1, col2, col3, col4, col5, col6, col7 = st.columns([2,0.5,2,0.5,2,0.5,2])  
        col1.info("CO(NH₂)₂")  
        col2.markdown("<div style='text-align:center;padding-top:8px'>+</div>", unsafe_allow_html=True)  
        col3.info("H₂O")  
        ...
        st.markdown("**Reaksi 3: Kesetimbangan Kromat ⇌ Dikromat (pengatur waktu pengendapan)**")  
        col1, col2, col3, col4, col5, col6, col7 = st.columns([2,0.5,2,0.5,2,0.5,2])  
        ...
        st.markdown("""  
        > **Penjelasan:**  
        > - **Suasana asam (awal):** kesetimbangan bergeser ke kanan → Cr₂O₇²⁻ dominan → Ba²⁺ **tidak** mengendap  
        > - **Setelah urea terhidrolisis (pH naik):** kesetimbangan bergeser ke kiri → CrO₄²⁻ dominan → BaCrO₄ **mulai mengendap**  
        >  
